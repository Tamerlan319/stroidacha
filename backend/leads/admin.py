...
class LeadAdmin(admin.ModelAdmin):
    ...
    # Телефона здесь нет: он хранится зашифрованным, и база по нему искать
    # не умеет (leads/crypto.py).
    search_fields = (
        "message",
        "project__title",
        "manager_comment",
    ...
    def get_readonly_fields(self, request, obj=None):
        ...
        # У существующей заявки телефон только для чтения: если ключ
        # расшифровки не задан, в поле показывается пояснение, и сохранение
        # формы не должно записать его поверх зашифрованного номера.
        if obj is not None:
            readonly.insert(0, "phone")
        return readonly
