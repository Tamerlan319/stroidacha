...
> **Отключён 22.09.2026.** Контейнеры `telegram-bot` и `xray` убраны из
> `docker-compose.prod.yml`: VPN-прокси перестал соединяться с Telegram, а
> трафик за рубеж владелец решил не пускать. Кнопка Telegram на сайте ведёт в
> обычный аккаунт — ссылка меняется в админке («Ссылки на соцсети»). Ниже —
> как бот был устроен, если его понадобится вернуть.
...
## YML-фид «Недвижимость» для Яндекса

`GET /api/feeds/realty.yml` (backend/catalog/feeds.py) — публичный,
всегда актуальный XML-фид всех активных домов и бань с ценами и фото,
для отдельного коммерческого блока в поиске Яндекса. Не связан с Яндекс
Бизнесом и не зависит от того, какой сайт сейчас официальный на карточке
организации — подключается напрямую через Вебмастер.

Подключить один раз:

1. [webmaster.yandex.ru](https://webmaster.yandex.ru) → сайт `brusodel.ru` →
   **«Фиды и ошибки»** → **«Загрузить фид»**.
2. Категория — **«Недвижимость»**.
3. URL фида: `https://brusodel.ru/api/feeds/realty.yml`.

Дальше Яндекс сам периодически перечитывает этот URL — обновлять вручную
не нужно, цены и наличие проектов в фиде всегда синхронны с каталогом.
